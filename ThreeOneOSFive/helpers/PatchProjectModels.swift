...
struct PatchDirectory: Codable, Identifiable, Hashable, Sendable {
    var id: UUID
    var bundleID: String
    var relativePath: String
...
/// Game/division inferred from the patch name.
enum PatchGame: String, CaseIterable, Identifiable, Sendable {
    case ffth = "FFTH"
    case ffm = "FFM"

    var id: String { rawValue }

    static func classify(_ name: String) -> PatchGame? {
        let normalized = name
            .uppercased()
            .replacingOccurrences(of: "_", with: " ")
            .replacingOccurrences(of: "-", with: " ")

        if normalized.range(of: #"\bFFTH\b"#, options: .regularExpression) != nil {
            return .ffth
        }
        if normalized.range(of: #"\bFFM\b"#, options: .regularExpression) != nil {
            return .ffm
        }
        return nil
    }
...
struct PatchPackageSummary: Equatable, Identifiable, Sendable {
...
struct EncodedPatchPackage: Sendable {
    let data: Data
    let contentKey: Data
}

struct DecodedPatchPackage: Sendable {
...
extension PatchPackageError: LocalizedError {
    var localizationKey: String {
        ...
        case .activePatchConflict: return "patch.error.active_conflict"
