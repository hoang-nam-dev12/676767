...
final class PatchProjectStore: ObservableObject {
    ...
    @discardableResult
    func importPackageAndWait(at sourceURL: URL, timeout: TimeInterval = 60) async -> Bool {
        guard !isBusy else { return false }

        let expectedID: UUID? = {
            guard let data = try? PatchProjectLibrary.readPackage(at: sourceURL),
        ...
        guard expectedID != nil else {
            return false
        }

        importPackage(at: sourceURL)
    ...
    func importPackage(data: Data, password: String? = nil) {
        guard !isBusy else { return }
        isBusy = true
