...
struct FeatureVisibility: Equatable {
    ...
    func isVisible(_ section: AppSection) -> Bool {
        ...
        case .home, .files, .patches, .settings, .at:
            return true
        }
    }
}

struct AppTabNavigationState: Equatable, Codable {
...
struct FileBrowserDestination: Hashable, Codable {
...
struct FilesTabState: Identifiable, Equatable, Codable {
    let id: UUID
...
struct FilesTabSession: Equatable, Codable {
...
// MARK: - Immediate local state persistence
/// Persists only lightweight UI/navigation state. Sensitive credentials and
/// patch payloads are intentionally excluded from this snapshot.
enum AppTabNavigationStore {
    private static let key = "app.navigation.state.v1"

    static func load() -> AppTabNavigationState? {
        guard let data = UserDefaults.standard.data(forKey: key) else { return nil }
        return try? JSONDecoder().decode(AppTabNavigationState.self, from: data)
    }

    static func save(_ state: AppTabNavigationState) {
