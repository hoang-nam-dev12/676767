...
enum PatchWorkspaceService {
    ...
    /// Patch workspaces are application-private data. They intentionally live
    /// outside Documents so iOS Files/iTunes document sharing does not expose them.
    /// The public method name is retained for source compatibility.
    static func documentsRootURL(fileManager: FileManager = .default) throws -> URL {
        ...
        let root = applicationSupport.appendingPathComponent(".PatchWorkspace", isDirectory: true)
    ...
    static func patchesRootURL(fileManager: FileManager = .default) throws -> URL {
        ...
        try fileManager.createDirectory(at: root, withIntermediateDirectories: true)
        migrateLegacyDocumentsPatchesIfNeeded(to: root, fileManager: fileManager)
        return root
    }

    /// One-time migration from the old Documents/Patches location.
    /// Existing workspaces are moved into Application Support and the old
    /// Documents/Patches directory is removed when it becomes empty.
    private static func migrateLegacyDocumentsPatchesIfNeeded(
        to destination: URL,
        fileManager: FileManager
    ) {
        let marker = destination.appendingPathComponent(".migration-v2-complete")
        ...
        do {
            ...
            try Data("v2".utf8).write(to: marker, options: [.atomic, .completeFileProtection])
        } catch {
            log("patch-workspace: legacy migration deferred – \(error.localizedDescription)")
        }
    }
